    ...
    */
    ...
    public R add(@RequestBody ShangpinOrderEntity shangpinOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,shangpinOrder:{}",this.getClass().getName(),shangpinOrder.toString());
            ...
            // 更新卖家余额（若商品未关联卖家则跳过，避免 NPE）
            if (shangpinEntity.getYonghuId() != null) {
                YonghuEntity yonghuEntity2 = yonghuService.selectById(shangpinEntity.getYonghuId());
                if (yonghuEntity2 != null) {
                    yonghuEntity2.setNewMoney(yonghuEntity2.getNewMoney() + shangpinOrder.getShangpinOrderTruePrice());
